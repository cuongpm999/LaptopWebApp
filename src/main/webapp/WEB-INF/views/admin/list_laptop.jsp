							...
							<table class="table">
								...
								<tbody id="myTable">
									<c:forEach var="laptop" items="${laptop }">
										<tr>
											<td>${laptop.laptopManufacturer.name }</td>
											<td style="width: 400px;">${laptop.name }</td>
											<td>${laptop.price }&nbsp;Đ</td>
											<td>${laptop.soLuongNhap }</td>
											<td>${laptop.khuyenMai }</td>
											<td><a href="/admin/edit-laptop/${laptop.id }"
												class="btn btn-primary"><i class="fas fa-edit"></i> Edit</a>
												<a href="javascript:void(0);"
												onclick="Shop.deleteForm('modalDeleteForm', ${laptop.id }, '/rest/api/laptop/delete')"
