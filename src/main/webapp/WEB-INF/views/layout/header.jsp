								...
								<c:choose>
									<c:when test="${not empty googleAcc }">
										<a class="dropdown-item-acc" href="#"><img
											...
											src="${googleAcc.getPicture() }"> <c:choose>
												<c:when test="${fn:length(googleAcc.getName())>15 }">
													${fn:substring(googleAcc.getName(), 0, 13)}...
												</c:when>
												<c:when test="${fn:length(googleAcc.getName())<=15 }">
													${googleAcc.getName() }
												</c:when>
											</c:choose> </a>
											<a class="dropdown-item-acc" href="/my-order"><img
											class="logInOut" alt="user"
											src="/img/icons8-cart-64.png">Đơn mua</a>
									...
									<c:when test="${not empty userDis}">
										...
										<a class="dropdown-item-acc" href="/user-details"><c:choose>
											...
											<c:choose>
												<c:when test="${fn:length(userDis.fullname)>15 }">
													${fn:substring(userDis.fullname, 0, 13)}...
												</c:when>
												<c:when test="${fn:length(userDis.fullname)<=15 }">
													${userDis.fullname }
												</c:when>
								...
								<c:choose>
									<c:when test="${not empty googleAcc }">
										<a class="dropdown-item" href="#"><img
											...
											<a class="dropdown-item" href="/my-order"><img
											class="logInOut" alt="user"
											src="/img/icons8-cart-64.png">Đơn mua</a>
										<sec:authorize access="hasAnyRole('ADMIN','USER')">
									...
									<c:when test="${not empty userDis}">
										...
										<a class="dropdown-item" href="/user-details"><c:choose>
												<c:when test="${empty userDis.userAttachment.name}">
													<img class="user-login-icon" alt="user"
														src="/img/anh-dai-dien-1.jpg">
												</c:when>
