...
public class UserService {
	...
	public List<UserStat> stats() {
		String sql = "SELECT tbl_user.*, A.SoTienMua FROM tbl_user, (SELECT SUM (all_money) AS SoTienMua, user_id FROM tbl_bill,tbl_payment WHERE tbl_bill.id = tbl_payment.bill_id GROUP BY user_id ORDER BY SoTienMua DESC) AS A WHERE tbl_user.id = A.user_id";
